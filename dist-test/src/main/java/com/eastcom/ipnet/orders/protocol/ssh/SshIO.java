package com.eastcom.ipnet.orders.protocol.ssh;
...
public abstract class SshIO {
	...
	public byte[] handleSSH(byte buff[]) throws IOException {
		...
		if (phase == PHASE_INIT) {
			...
			while (boffset < buff.length) {
				...
				idstr += (char) b;
				if (b == '\n') {
					// System.out.println("remote idstr = " + idstr);
	...
	private String handlePacket2(SshPacket2 p) throws IOException {
		...
		case SSH2_MSG_KEXINIT: {
			...
			System.out.println("KEXINIT- " + kexalgs);
			String hostkeyalgs = p.getString();
			System.out.println("KEXINIT- " + hostkeyalgs);
			String encalgc2s = p.getString();
			System.out.println("KEXINIT- " + encalgc2s);
			String encalgs2c = p.getString();
			System.out.println("KEXINIT- " + encalgs2c);
			String macalgc2s = p.getString();
			System.out.println("KEXINIT- " + macalgc2s);
			String macalgs2c = p.getString();
			System.out.println("KEXINIT- " + macalgs2c);
			String compalgc2s = p.getString();
			System.out.println("KEXINIT- " + compalgc2s);
			String compalgs2c = p.getString();
			System.out.println("KEXINIT- " + compalgs2c);
			String langc2s = p.getString();
			System.out.println("KEXINIT- " + langc2s);
			String langs2c = p.getString();
			System.out.println("KEXINIT- " + langs2c);
			fupp = p.getBytes(1);
			System.out.println("KEXINIT- first_kex_follows: " + fupp[0]);
			/* int32 reserved (0) */
	...
	private String handlePacket1(SshPacket1 p) throws IOException { // the
		...
		case SSH_MSG_DISCONNECT:
			...
			disconnect();
			System.out.println("SSH_MSG_DISCONNECT: " + str);
			return str;
		...
		case SSH_SMSG_SUCCESS:
			...
			if (lastPacketSentType == SSH_CMSG_USER) {
				...
				Send_SSH_CMSG_REQUEST_PTY(); // request a pseudo-wrapper
			...
			if (lastPacketSentType == SSH_CMSG_REQUEST_PTY) {// pty accepted
				...
				if (idstr.startsWith("SSH-1.5"))
				{
					//����SSH-1.5 �����ȷ���SSH_CMSG_EXEC_SHELL
				...
				if (!idstr.startsWith("SSH-1.5"))
				{
					//���ڷ�SSH-1.5�汾 ,��ʱ����SSH_CMSG_EXEC_SHELL
					Send_SSH_CMSG_EXEC_SHELL(); 
				}
	 ...
	 * vt100) 32-bit int wrapper height, rows (e.g., 24) 32-bit int wrapper
	 * width, columns (e.g., 80) 32-bit int wrapper width, pixels (0 if no
	 * graphics) (e.g., 480)
	 */
	...
	private String Send_SSH_CMSG_REQUEST_PTY() throws IOException {
		...
		p.putInt32(0); // Int32 x pixels
		p.putInt32(0); // Int32 y pixels
		p.putByte((byte) 0); // Int8 wrapper modes
		sendPacket1(p);
		return "";
	}
