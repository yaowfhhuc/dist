...
public abstract class Channel implements Runnable {
	...
	static Channel getChannel(String type) {
		...
		/*
		 * if(type.equals("exec")){ return new ChannelExec(); }
		 * if(type.equals("x11")){ return new ChannelX11(); }
		 * if(type.equals("direct-tcpip")){ return new ChannelDirectTCPIP(); }
		 * if(type.equals("forwarded-tcpip")){ return new
		 * ChannelForwardedTCPIP(); } if(type.equals("sftp")){ return new
		 * ChannelSftp(); } if(type.equals("subsystem")){ return new
		 * ChannelSubsystem(); }
		 */
		return null;
	}
	...
	public void connect() throws SshException {
		...
		try {
			...
			int retry = 1000;
			while (this.getRecipient() == -1 && session.isConnected()
					&& retry > 0) {
				try {
					Thread.sleep(50);
				} catch (Exception ee) {
	...
	/*
	 * public void finalize() throws Throwable{ disconnect(); super.finalize();
	 * session=null; }
	 */
