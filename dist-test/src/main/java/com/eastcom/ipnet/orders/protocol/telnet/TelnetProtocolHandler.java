package com.eastcom.ipnet.orders.protocol.telnet;
...
public abstract class TelnetProtocolHandler {
	...
	private final static byte TELOPT_TTYPE = (byte) 24; /* wrapper type */
	...
	/**
	 * Get the current wrapper type for TTYPE telnet option.
	 * 
	 * @return the string id of the wrapper
	 */
	protected abstract String getTerminalType();

	/**
	 * Get the current window size of the wrapper for the NAWS telnet option.
	 * 
	 * @return the size of the wrapper as Dimension
	 */
	...
	private void handle_sb(byte type, byte[] sbdata) throws IOException {
		...
		case TELOPT_TTYPE:
			...
			if ((sbdata.length > 0) && (sbdata[0] == TELQUAL_SEND)) {
				...
				/*
				 * FIXME: need more logic here if we use more than one wrapper
				 * type
				 */
