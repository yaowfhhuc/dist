...
class IdentityFile implements Identity {
	...
	IdentityFile(String identity, JSch jsch) throws SshException {
		...
		try {
			...
			int i = 0;
			while (i < len) {
				if (buf[i] == 'B' && buf[i + 1] == 'E' && buf[i + 2] == 'G'
						&& buf[i + 3] == 'I') {
					i += 6;
					if (buf[i] == 'D' && buf[i + 1] == 'S' && buf[i + 2] == 'A') {
						...
						throw new SshException("invaid privatekey: " + identity);
					}
					i += 3;
					continue;
				}
				if (buf[i] == 'C' && buf[i + 1] == 'B' && buf[i + 2] == 'C'
						&& buf[i + 3] == ',') {
					i += 4;
			...
			while (i < len) {
				if (buf[i] == 0x0a) {
					...
					System.arraycopy(buf, i + 1, buf, i - (xd ? 1 : 0), len - i
							- 1 - (xd ? 1 : 0));
					if (xd)
						len--;
					len--;
			...
			if (encoded_data.length > 4
				...
				if (cipher.equals("3des-cbc")) {
					_buf.getInt();
					byte[] foo = new byte[encoded_data.length
							- _buf.getOffSet()];
					_buf.getByte(foo);
					encoded_data = foo;
					encrypted = true;
					throw new SshException("unknown privatekey format: "
							+ identity);
	...
	public boolean setPassphrase(String _passphrase) throws SshException {
		/*
	...
	byte[] getPublicKeyBlob_rsa() {
		if (e_array == null)
			return null;
		Buffer buf = new Buffer("ssh-rsa".length() + 4 + e_array.length + 4
				+ n_array.length + 4);
	...
	byte[] getPublicKeyBlob_dss() {
		if (P_array == null)
			return null;
		Buffer buf = new Buffer("ssh-dss".length() + 4 + P_array.length + 4
				+ Q_array.length + 4 + G_array.length + 4 + pub_array.length
				+ 4);
	...
	byte[] getSignature_rsa(byte[] data) {
		try {
			 ...
			 * goo[3]=(byte)(session.getSessionId().length); rsa.update(goo);
	...
	byte[] getSignature_dss(byte[] data) {
		/*
		 * byte[] foo; int i; System.out.print("P "); foo=P_array; for(i=0; i<foo.length;
		 * i++){ System.out.print(Integer.toHexString(foo[i]&0xff)+":"); }
		 * System.out.println(""); System.out.print("Q "); foo=Q_array; for(i=0;
		 * i<foo.length; i++){
		 * System.out.print(Integer.toHexString(foo[i]&0xff)+":"); }
		 * System.out.println(""); System.out.print("G "); foo=G_array; for(i=0;
		 * i<foo.length; i++){
		 * System.out.print(Integer.toHexString(foo[i]&0xff)+":"); }
		 * System.out.println("");
		 */
		...
		try {
			 ...
			 * goo[1]=(byte)(session.getSessionId().length>>>16);
			 * goo[2]=(byte)(session.getSessionId().length>>>8);
			 * goo[3]=(byte)(session.getSessionId().length); dsa.update(goo);
		...
		try {
			...
			if (encrypted) {
				if (keytype == OPENSSH) {
					...
					cipher.update(encoded_data, 0, encoded_data.length, plain,
							0);
				} else if (keytype == FSECURE) {
			...
			index += length;
			/*
			 * System.out.println("int: N len="+length); for(int i=0; i<n_array.length;
			 * i++){ System.out.print(Integer.toHexString(n_array[i]&0xff)+":"); }
			 * System.out.println("");
			 */
			index++;
			length = plain[index++] & 0xff;
			...
			index += length;
			/*
			 * System.out.println("int: E len="+length); for(int i=0; i<e_array.length;
			 * i++){ System.out.print(Integer.toHexString(e_array[i]&0xff)+":"); }
			 * System.out.println("");
			 */
			index++;
			length = plain[index++] & 0xff;
			...
			index += length;
			/*
			 * System.out.println("int: D len="+length); for(int i=0; i<d_array.length;
			 * i++){ System.out.print(Integer.toHexString(d_array[i]&0xff)+":"); }
			 * System.out.println("");
			 */
			...
			index += length;
			/*
			 * System.out.println("int: P len="+length); for(int i=0; i<p_array.length;
			 * i++){ System.out.print(Integer.toHexString(p_array[i]&0xff)+":"); }
			 * System.out.println("");
			 */
			index++;
			length = plain[index++] & 0xff;
			...
			index += length;
			/*
			 * System.out.println("int: q len="+length); for(int i=0; i<q_array.length;
			 * i++){ System.out.print(Integer.toHexString(q_array[i]&0xff)+":"); }
			 * System.out.println("");
			 */
			index++;
			length = plain[index++] & 0xff;
			...
			index += length;
			/*
			 * System.out.println("int: dmp1 len="+length); for(int i=0; i<dmp1_array.length;
			 * i++){
			...
			index += length;
			/*
			 * System.out.println("int: dmq1 len="+length); for(int i=0; i<dmq1_array.length;
			 * i++){
			...
			index += length;
			/*
			 * System.out.println("int: iqmp len="+length); for(int i=0; i<iqmp_array.length;
			 * i++){
