...
public class SocketServerChannelTest {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(SocketServerChannelTest.class);
    ...
    private static void readSocketChannel(SelectionKey readyKey) throws Exception {
        ...
        if(URLDecoder.decode(message.toString(), "UTF-8").indexOf("over") != -1) {
            ...
            LOGGER.info("端口:" + resoucePort + "客户端发来的完整信息======completeMessage : " + URLDecoder.decode(completeMessage.toString(), "UTF-8"));
        	...
        	LOGGER.info("端口:" + resoucePort + "客户端信息还未接受完，继续接受======message : " + URLDecoder.decode(message.toString(), "UTF-8"));
